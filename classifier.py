...
random_param_dist = {
    ...
    "NN": {
        ...
        "classifier__validation_fraction": uniform(
            0.05,
            0.2,
        ),  # Fraction of training data for validation
        "classifier__n_iter_no_change": randint(
            5,
            10,
...
class Classifiers:
    def generate_init_models(self):
        init_config = {
            "LR": {
                ...
                "config": self.config.get("classifiers")
                .get("init_model_config")
                .get("LR"),
            },
            "SVC": {
                "model": SVC,
    ...
    def generate_best_model_config(self):
        ...
        for model_name, model_info in self.init_models.items():
            best_config[model_name] = {
                ...
                "config": self.config.get("best_model_config")
                .get(model_name)
                .get("config")
                .get("val_score", {}),
                "val_score": self.config.get("best_model_config")
                .get(model_name)
                .get("val_score", 0.0),
            }
        return best_config
    ...
    def __init__(self, config):
        ...
        self.skfold = StratifiedKFold(
            n_splits=self.config.get("classifiers").get("k_folds"),
            shuffle=True,
            random_state=42,
        )
    ...
    def fit_and_eval(self, preprocess_pipelines, X_train, y_train):
        ...
        for preprocess_type, p_pipeline in preprocess_pipelines.items():
            printd(
                f"Training {len(self.init_models)} models for {preprocess_type}: {self.init_models.keys()}",
                file=self.file,
            )
            for algo, model_info in tqdm(self.init_models.items()):
                model_estimator_step = (
                    "classifier",
                    model_info.get("model")(**model_info.get("config")),
                )
                pipeline_with_estimator = Pipeline(
                    p_pipeline.steps + [model_estimator_step],
                )
                algo_results = cross_validate(
                    pipeline_with_estimator,
                    X_train,
                    y_train,
                    cv=self.skfold,
                ...
                algo_results_df.columns = [
                    col.replace("test_", "val_") for col in algo_results_df.columns
                ]
                results.append(algo_results_df)

        new_eval_df = pd.concat(results, ignore_index=True).sort_values(
            by=["algo", "preprocess_type"],
        )
    ...
    def fit_and_eval_test_set(
        ...
        for preprocess_type, p_pipeline in preprocess_pipelines.items():
            printd(
                f"Training {len(self.init_models)} models for {preprocess_type}: {self.init_models.keys()} on full train set",
                file=self.file,
            )
            for algo, model_info in tqdm(self.best_model_config.items()):
                ...
                pipeline_with_estimator = Pipeline(
                    p_pipeline.steps + [model_estimator_steps],
                )
        ...
        if self.test_eval_df is None:
            self.test_eval_df = pd.concat(results, ignore_index=True).sort_values(
                by=["algo", "preprocess_type"],
            )
        else:
            new_test_eval_df = pd.concat(results, ignore_index=True)
            self.test_eval_df = pd.concat(
                [self.test_eval_df, new_test_eval_df],
                axis=0,
                ignore_index=True,
            ).sort_values(
                by=["algo", "preprocess_type"],
    ...
    def check_and_update_best_config(
        self,
        algo,
        best_params,
        best_score,
        ...
        """
        old_score = (
            self.config.get("best_model_config")
            .get(algo, {})
            .get("best_score_", float("-inf"))
        )
        if best_score > old_score:
            ...
            printd(
                f"Updated configuration for {algo} saved to {file_path}",
                file=self.file,
            )
        else:
            printd(
    ...
    def plot_val_metric_comp(self):
        ...
        for col in metrics:
            self.agg_eval_df[[f"{col}_mean", f"{col}_uncertainty"]] = self.agg_eval_df[
                col
            ].apply(
        ...
        x = np.arange(
            len(self.agg_eval_df["algo"].unique()),
        )  # Position of bars for each group
        ...
        for metric in metrics:
            ...
            ax.set_title(
                f'Comparison of {metric.replace("_", " ").capitalize()} with Uncertainty',
            )
    ...
    def plot_test_metric_comp(self):
        ...
        for i, metric in enumerate(metrics):
            ax = axes[i]
            ax.bar(
                self.test_eval_df["algo"],
                self.test_eval_df[metric],
                color=colors,
                alpha=0.8,
            )
    ...
    def hyper_parm_tuning(
        ...
        for algo in algos:
            ...
            model_estimator_steps = (
                "classifier",
                self.init_models.get(algo).get("model")(
                    **self.init_models.get(algo).get("config"),
                ),
            )
            pipeline_with_estimator = Pipeline(
                preprocess_pipeline.steps + [model_estimator_steps],
            )
            random_search = RandomizedSearchCV(
                pipeline_with_estimator,
                param_distributions=random_param_dist.get(algo),
                n_iter=n_iter,
                cv=self.config.get("classifiers").get("k_folds"),
                scoring="f1",
                n_jobs=-1,
            )
            ...
            self.check_and_update_best_config(
                algo,
...
if __name__ == "__main__":
    ...
    k_fold_eval, agg_eval = c.fit_and_eval(
