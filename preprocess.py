...
class AgeBinner(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        labels = [f'{int(bins[i])}_{int(bins[i + 1]) - 1}' for i in range(self.num_bins)]
...
class PdaysBinner(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        binned = pd.cut(X_copy.clip(upper=self.max_value), bins=bins, labels=labels, right=False)
        binned_encoded = pd.get_dummies(binned, prefix='pdays', dtype=float)

        # Feature for pdays > max bin value
        binned_encoded[f'pdays_gt_{int(bins[-1] - 1)}'] = (X_copy > (bins[-1] - 1)).astype(float)
...
class DropFeatures(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        X_dropped = X.drop(columns=[col for col in self.features_to_drop if col in X.columns], axis=1)
        return X_dropped
...
class Preprocess:
    def __init__(self, config, df):
        ...
        self.continuous_features = self.df.select_dtypes(include=['int64', 'float64']).columns
    ...
    def plot_distribution(self, raw_df, split):
        ...
        for feature in self.continuous_features:
            ...
            sns.kdeplot(data=raw_df[raw_df[self.target_col] == 1], x=feature, label='Target: Yes', fill=True,
                        alpha=0.5, ax=axes[0])

            sns.kdeplot(data=raw_df[raw_df[self.target_col] == 0], x=feature, label='Target: No', fill=True,
                        alpha=0.5, ax=axes[0])
            ...
            sns.boxplot(x=self.target_col, y=feature, data=raw_df, palette='Set2', ax=axes[1], hue=self.target_col, )
        ...
        for feature in self.categorical_features:
            ...
            proportions = raw_df.groupby(feature)[self.target_col].value_counts(normalize=True).unstack()
            ...
            # Save the plot
            plot_file = f"{plot_dir}/{feature}.png" if plot_dir.endswith('/') else f"{plot_dir}{feature}.png"
    ...
    def generate_raw_data_info(self, raw_df, split):
        ...
        print(raw_df[self.config.get("preprocess").get("target_col")].value_counts(), file=self.file)
        # adding unique values to info
        unique_values = {col: (raw_df[col].value_counts(normalize=True).to_dict()) for col in raw_df.columns}
    ...
    def _get_feature_names(self, preprocessor):
        ...
        onehot_columns = preprocessor.named_transformers_['cat_features']['onehot_encoder'].get_feature_names_out(
            self.categorical_features)
        age_bin_columns = preprocessor.named_transformers_['age']['age_binner'].get_feature_names_out()
        cont_features_without_age = [i for i in self.continuous_features if
                                     i not in [self.config.get("preprocess").get("age_bin").get("col")]]
    ...
    def gen_basic_pipeline(self, features_to_ignore=None):
        if features_to_ignore is None:
            features_to_ignore = []
        categorical_features = [i for i in self.categorical_features if i not in features_to_ignore]
        continuous_features = [i for i in self.continuous_features if i not in features_to_ignore]
        ...
        continuous_pipeline = Pipeline([
            ('passthrough', 'passthrough')  # No transformation for numerical columns
        ])

        preprocessor = ColumnTransformer(
            transformers=[
                ('cat_features', categorical_pipeline, categorical_features),
                ('cont_features', continuous_pipeline, [i for i in continuous_features if i not in [
                    self.config.get("preprocess").get("age_bin").get("col"), "pdays"]]),
            ]
        )

        full_pipeline = Pipeline([
            ('preprocessing', preprocessor),  # Categorical and numerical preprocessing
            ('scaling', StandardScaler()),  # Apply scaling to all features at the end
        ])
    ...
    def gen_pipeline_with_age_pdays_binned(self, features_to_ignore=None):
        ...
        age_binning_pipeline = Pipeline([
            ('age_binner', AgeBinner(
                ...
                num_bins=self.config.get("preprocess").get("age_bin").get("num_bins", 11),
                min_age=self.config.get("preprocess").get("age_bin").get("min_age", 17),
                max_age=self.config.get("preprocess").get("age_bin").get("max_age", 98)
            ))
        ])
        ...
        new_transformers = preprocessor.transformers + [
            ('age', age_binning_pipeline, self.config.get("preprocess").get("age_bin").get("col")),
    ...
    def gen_pipeline_with_pca(self, pca_variance_to_keep, features_to_drop=None):
        ...
        feature_drop_pipeline = self.gen_pipeline_with_feature_drop(features_to_drop).steps
    ...
    def gen_pipeline_with_smote(self, pca_variance_to_keep, features_to_drop=None):
        ...
        if pca_variance_to_keep:
            pca_step = self.gen_pipeline_with_pca(pca_variance_to_keep, features_to_drop=None).steps[-1]
    ...
    def gen_pipeline_with_smoteenn(self, pca_variance_to_keep, features_to_drop=None):
        prev_pipeline = self.gen_pipeline_with_feature_drop(features_to_drop).steps
        smoteenn_step = ('smoteenn', SMOTEENN(sampling_strategy='auto', random_state=42))
...
def download_data(config, n_rows=None):
    ...
    urllib.request.urlretrieve(config.get("preprocess").get("data_url"), local_path + "data.zip")
    with zipfile.ZipFile(local_path + "data.zip", 'r') as zip_ref:
        zip_ref.extractall(local_path)

    raw_df = pd.read_csv(local_path + config.get("preprocess").get("zip_file_struct"), sep=config.get("preprocess").get("data_sep"))
    if n_rows:
